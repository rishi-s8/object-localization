...
encoded.load_weights("./encoder.h5")

encoded.trainable=False

filepath='./regressor/model.h5'
filepath2='./regress/finalEpoch.h5'
...
for y in range(0,13000, 1000):
	...
	history=model.fit(x=x_train, y=y_train, batch_size=10, epochs=epochs, validation_split=0.05, shuffle=True, verbose=1, callbacks=callbacks_list)
	plt.clf()
	plt.set_yscale = ('log')
	...
	plt.savefig('./regressor/plots/' + str(y) +'.png')
